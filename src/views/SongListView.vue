<script setup>
  ...
  import {getSonglistInfoReq} from '../assets/utils/api.js'
  ...
  function convertToTabelData(songlist) {
    ...
    for (let i=0; i < songlist.length; i++) {
      data[i] = {
        ...
        songmid: songlist[i].songmid,
        songname: songlist[i].songname,
        singer: songlist[i].singer[0].name,
        albumname: songlist[i].albumname,
        interval: songlist[i].interval,
        pay_play: songlist[i].pay.payplay == 1
      }
    }
